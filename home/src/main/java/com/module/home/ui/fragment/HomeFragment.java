...
public class HomeFragment extends BaseLazyFragment<HomeFragmentHomeMainBinding> {
    ...
    protected void init() {
        ...
        homeViewModel.getMessageLiveData().observe(getViewLifecycleOwner(), s -> SnackBarBuilder.getInstance().builderLong(context, s));
        homeViewModel.captcha(new CaptchaBean("1", "```"));
        homeViewModel.getCaptchaLiveData().observe(getViewLifecycleOwner(), o -> {
